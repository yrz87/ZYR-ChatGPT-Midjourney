...
async function verificationToken<T = any>(options: Partial<AuthInfo>) {
  ...
  try {
    const response  = await verificationCode({token:systemToken.value}) as ResponseData;
    ...
    if (response && response.success) {
      ...
      if(response.data.userIdentify == 1){//包月
          const expressDate = response.data.expressDate == '0'?"永久":formatDate(response.data.expressDate)
          const expressDateNumber = response.data.expressDate == '0'?0:new Date(response.data.expressDate).getTime()
          const vipChatgpt4Num = response.data.vipChatgpt4Num == null?0:response.data.vipChatgpt4Num
      ...
      }else if(response.data.userIdentify == 0){//购买次数
          tokenStore.updateSetting({
...
<template>
  <div class="p-4 space-y-5 min-h-[200px]">
    ...
    <div class="text-lg font-bold"  v-if="freeNum>0 && openFreeNum && !tokenVerification">
    ...
    <div class="space-y-6">
      <div class="flex items-center space-x-4">
        ...
        <div class="flex-1">
          <NInput v-model:value="systemToken" placeholder="密钥不能为空,请输入正确的密钥" />
        </div>
        <NButton size="tiny" text type="primary" @click="updateSettings({ systemToken })" :style="{ color: loading ? '#ccc' : '' }">
          {{ $t('common.save') }}
        </NButton>
      </div>
      ...
      <div class="p-2 space-y-2 rounded-md bg-neutral-100 dark:bg-neutral-700">
        <div class="text-lg font-bold" v-if="authInfo.userIdentify == 0">
          您剩余积分为:<text style="color:red">{{authInfo.surplusNum}}</text>,已经使用积分为:<text style="color:red">{{authInfo.useNum}}</text>
        </div>
        <div class="text-lg font-bold" v-if="authInfo.userIdentify == 1">
          您的有效期为:<text style="color:red">{{authInfo.expressDate}}</text>,注册时间为:<text style="color:red">{{authInfo.startDate}}</text>
        </div>
